...
- [x] **[iOS] iframe から鍵情報が見えない（WebKit のストレージ分離）— 真因を特定し修正済み** — 2026-09 の iPhone 実機ログ（iOS 18.7 / Safari 26.6.1、`?debug=1` の計測モードで採取）で確定。**ストレージ分離自体は起きているが cookie ブリッジは正しく機能していた**: grant 後は `manager: storage=CookieStorage nosskey_pwk=180B/direct` と鍵が読めており、`localStorage` は grant 後も `(none)` のまま（WebKit が localStorage を unpartition しないことも実測で確認）。真因は**タイミング**だった — `nosskey:ready` を同期で送るため親の最初のリクエストが回復前に届き、host が `NO_KEY` を即答していた。ユーザーが許可した頃には親の Promise は reject 済みで、「アクセスを許可しました」と「ログインできません」が同時に出る。WebKit は `requestStorageAccess()` にユーザージェスチャを必須とするため iOS では必ずこの順序になる。**対応**: `NosskeyIframeHost` に `storageReady`（ストレージ解決まで ready を遅らせる）と `onKeyUnavailable`（`NO_KEY` を即答せず回復を待ち、その後 `hasKeyInfo()` を再判定する）を追加。いずれも省略可能で、省略時の挙動は従来どおり。当初の対応方針だった「iOS では iframe 内リカバリを諦めて `window.open()` ポップアップ署名へ倒す」は不要になったため撤回する（P3 の Phase 5-A も前提が変わる）。
