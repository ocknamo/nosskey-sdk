...
- [x] **未消費 PRF キャッシュ（`#pendingPrfByCredId`）の滞留対策** — TTL 自動ゼロ化＋明示クリア API の両方で対応。(1) `createPasskey()` でキャッシュ退避時に `setTimeout` で TTL（`PENDING_PRF_TTL_MS` = 60 秒、barrel export・Node では `unref` でプロセス維持を回避）の自動ゼロ化タイマーを仕掛け、未消費のまま放置された場合（例外・画面離脱）でも 32 byte 秘密値が heap に滞留し続けないようにした。消費時（`#consumePendingPrf`）・上書き時はタイマーを解除。(2) 公開 API `clearPendingPrf(credentialId?)` を追加（省略時は全エントリ破棄、`NosskeyManagerLike` にも追加）し、`clearCurrentKeyInfo()`（ログアウト）と `clearStoredKeyInfo()`（完全ワイプ）からも自動で呼ぶようにした。共通実装は `#clearPendingPrfEntry`（タイマー解除＋両バッファゼロ化＋Map 削除）。svelte-app の `AuthScreen` は `createNew`/`importExisting` の catch（createPasskey 成功後の例外離脱経路）で `clearPendingPrf()` を即時呼び出し。ja/en interface 文書にキャッシュ管理メソッドとして追記。spec に TTL 満了・明示クリア・logout 経由破棄の 3 ケースを追加。
