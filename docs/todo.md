...
- [x] **nosskey-iframe と他アプリを iframe で組み合わせて1つの Nostr アプリとして構築** — nosskey-iframe を NIP-07 署名プロバイダとして埋め込みつつ、別の Nostr クライアント（タイムライン・DM 等）を同一ページに並置し、単一 Nostr アプリとして動作させる統合パターンの設計と参照実装。導入障壁を下げる効果が大きい。
- [ ] **[iOS] iframe から鍵情報が見えない時間が長い（WebKit のストレージ分離）— 「登録したのにログインできない」の最有力仮説** — 2026-09 の実機検証で「登録直後の 2 回目 `credentials.get()` が通らない」仮説（アーカイブ済み）が否定されたため、元の報告の真因としては本項目が最有力になった。次の切り分けは **iframe 埋め込み経由での iPhone 実機検証**。計測手段は導入済み（`?debug=1` でオンページのコンソールパネルと診断スナップショットが出る。手順は `docs/ja/ios-iframe-diagnostics.ja.md`）。 WebKit はクロスオリジン iframe の localStorage を partition し、Storage Access API の grant 後も unpartition されるのは cookie のみ。`examples/svelte-app/src/components/screens/IframeHostScreen.svelte` の `detectInitialState()` はマウント時に**ユーザージェスチャなしで** `document.requestStorageAccess({ all: true })` を呼ぶが、WebKit は原則ジェスチャ必須のため iOS では毎回失敗し、`partitioned` カードのタップを経由しないと鍵が見えない。加えて WebKit の 3rd-party ストレージパーティションは実質エフェメラル（タブを閉じると消える）で、ITP はスクリプト書き込みの localStorage / cookie を 7 日で失効させる。既存の cookie ブリッジ（`services/cookie-storage.ts` / `multi-storage.ts`、Phase 5-A）だけでは埋まらない。鍵が見えない状態はユーザーを汎用ログイン導線へ誘導するため、PR で修正した「ログイン＝復元」の設計と合わせて重要度が高い。**対応方針**: iOS では iframe 内リカバリを諦め、`window.open()` ポップアップ署名方式（P3 の Phase 5-A）へ倒す。あわせて `docs/{ja,en}/iframe-integration` の「Safari / iOS は非対応」記述を実態に合わせて見直す。
- [ ] **console へ例外オブジェクトを丸ごと出している箇所の棚卸し** — 計測モード（`?debug=1`）のパネルはアプリ全体の console を取り込み、ログは全文を貼り付けて共有される前提のため、例外オブジェクトをそのまま出す箇所は秘密値の持ち出し経路になる。実証済みの 1 件（`bech32` の `decode()` が失敗メッセージへ入力文字列＝打ち間違えた nsec を埋め込む）は `examples/svelte-app/src/utils/bech32-converter.ts` で例外名だけに落として対応済み。**残り**: `components/settings/ImportKeyInfo.svelte`（3 箇所）、`components/settings/ExportSecretKey.svelte`、`packages/nosskey-sdk/src/nosskey.ts` の `Failed to parse stored NostrKeyInfo`。いずれも例外メッセージに秘密値が載る証拠は未確認だが、秘密鍵を扱う導線なので方針を揃えること。
- [ ] **計測モード（`?debug=1`）を撤去する — 期限: 上記「[iOS] iframe から鍵情報が見えない」P1 項目のクローズ時** — 本番 `nosskey.app` と GitHub Pages のバンドルに同梱される。ホスト制限（localhost のみ受理等）をかけない理由は、計測対象が本番の実機そのものであり、制限すると計測目的を果たせなくなるため。受け入れているリスクは次の 3 点: (a) 埋め込み親が任意に `debug=1` を付けると署名 iframe が自動表示され、iframe 内にオーバーレイが出る（パネル内容はクロスオリジンで親から読めないため情報漏洩は無く、実害は見た目の変更に留まる）、(b)「`?debug=1` を付けてログを全文貼ってください」というソーシャルエンジニアリング導線（緩和策はパネル冒頭の共有前確認の警告行のみ）、(c) 例外オブジェクトがパネルへ載る経路（下記の棚卸し項目。Error 系は console-daijin が `{}` としか描画しないため実リスクは限定的）。**P1 がクローズしたら、撤去するか継続するかをこの 3 点を再評価したうえで判断し、結論を本項目に追記すること。**
- [ ] **CI に `examples/parent-sample` のビルド（または `tsc --noEmit`）を追加** — root の `npm run build` は `nosskey-sdk` / `nosskey-iframe` / `svelte-app` のみで、parent-sample は `deploy.yml` でしかビルドされない。型崩れが deploy 時に初めて落ちる。なお parent-sample には既存の `TS18047: 'next' is possibly 'null'`（`src/main.ts`）が残っており、CI へ入れるならこれも併せて解消が必要。
...
- [ ] **`loginWithPasskey()` の assertion で wrap salt も同時 eval する** — 現状 `loginWithPasskey()` は標準 salt のみ eval するため、直接モードは「ログイン 1 UV → 初回署名 0 UV」だが、wrap モード（インポート鍵）は「ログイン 1 UV → 初回署名でもう 1 UV」になる。`prf.eval.second = WRAP_SALT` を同時に要求し、wrap 一致時はその場で復号して派生キャッシュへ載せれば 1 UV に揃えられる。`getPrfSecret()` が `first` しか受け付けないため、prf-handler の拡張が必要。`second` 非対応のオーセンティケータでは `results.second` が undefined になるだけなので、現行動作へ安全に degrade する。
...
- [ ] **Safari 向け `window.open()` フォールバック（Phase 5-A）** — Safari PRF サポート安定後に対応。P1 の「[iOS] iframe から鍵情報が見えない時間が長い」がこの方式を要求しており、着手時はそちらの調査結果を前提にすること。
...
- [x] **[iOS] 登録直後の 2 回目 `credentials.get()` が通らない疑い — 実機検証により仮説を否定** — iframe 埋め込みアプリで Android は登録できるが iOS Safari で「登録したのにログインできない」との報告に対する第一仮説だった。仮説は「**WebKit は `create()` 時に `prf.results` を返さない**ため `createPasskey()` が PRF を退避できず、`createNostrKey()` / `importNostrKey()` が `getPrfSecret()` へフォールバックする。その `navigator.credentials.get()` は `create()` の await が解けた後＝ユーザージェスチャ失効後の発行になり `NotAllowedError` になる」というもの。**2026-09 の iPhone 実機検証（svelte-app 単体、iframe なし）で否定された**: 新規登録時の Face ID 確認は **1 回だけ**で、パスキー作成から鍵作成・ログインまで完走する。つまり `create()` は `prf.results` を返しており、フォールバック自体が発生していない。検証結果は `docs/{ja,en}/prf-support-tables` の「登録時 PRF と user gesture」節に記録した。**残: 元の報告の真因は未特定** — create で PRF が取れる以上 iframe 経由でもフォールバックは起きないため、下記「[iOS] iframe から鍵情報が見えない時間が長い（WebKit のストレージ分離）」が有力。`NostrKeyInfo` は保存されているが iframe の partitioned（実質エフェメラル）localStorage に入って消えている、という筋書きは「パスキーは iCloud キーチェーンに残っているのにログインできない」という症状と整合する。次は **iframe 埋め込み経由での iPhone 実機検証**を行い、そちらの項目で追跡すること。
