...
- [ ] **計測モード（`?debug=1`）の撤去期限を決める** — 本番 `nosskey.app` と GitHub Pages のバンドルに同梱される。埋め込み親が任意に `debug=1` を付けると iframe が自動表示される（パネル内容はクロスオリジンで親から読めないため実害は低い）。iOS のストレージ分離調査が終わり次第、フラグごと撤去するか、撤去しない判断とその理由を記録する。
