...
- [ ] **[iOS] 登録直後の 2 回目 `credentials.get()` が通らない疑い（「パスキー登録後にログインできない」）** — iframe 埋め込みアプリで Android は登録できるが iOS Safari で「登録したのにログインできない」との報告があり、その第一仮説。`createPasskey()` は create 時に `prf.eval.first/second` を先取りしてキャッシュする設計（`packages/nosskey-sdk/src/nosskey.ts` の `createPasskey`、動機は Android Chrome の `NotReadableError` 回避）だが、**WebKit が登録時に `prf.results` を返さない場合**は `createNostrKey()` / `importNostrKey()` が `getPrfSecret()` へフォールバックし（`nosskey.ts` の pendingPrf 消費分岐、`prf-handler.ts` の `getPrfSecret`）、**`create()` の await が解けた後**＝ユーザージェスチャ失効後に `navigator.credentials.get()` を発行する。WebKit は WebAuthn 呼び出しにユーザージェスチャを要求するため `NotAllowedError` になる可能性が高い。症状（パスキーは iCloud キーチェーンに作成されたのにアプリ側に `NostrKeyInfo` が保存されない＝以後ログインできない）と整合する。**確認手順**: iOS 実機 + Mac の Web インスペクタで、(1) `createPasskey` 直後の `getClientExtensionResults().prf`（`results` の有無）、(2) 直後の `credentials.get()` の例外名を採取。**対応方針**: create で PRF が返らなかった場合は自動で get へ進まず、「もう一度タップして鍵を作成」のようにユーザージェスチャ内で `credentials.get()` を発行する 2 タップ経路を用意する（`examples/svelte-app/src/components/screens/AuthScreen.svelte` の `createNew` / `importExisting`）。
