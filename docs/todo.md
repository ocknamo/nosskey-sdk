...
- [ ] **[iOS] 登録直後の 2 回目 `credentials.get()` が通らない疑い（「パスキー登録後にログインできない」）— 実装済み・実機検証待ち** — iframe 埋め込みアプリで Android は登録できるが iOS Safari で「登録したのにログインできない」との報告への対応。原因の第一仮説は「**WebKit は `create()` 時に `prf.results` を返さない**（`enabled` のみ）ため `createPasskey()` が PRF を退避できず、`createNostrKey()` / `importNostrKey()` が `getPrfSecret()` へフォールバックする。その `navigator.credentials.get()` は `create()` の await が解けた後＝ユーザージェスチャ失効後の発行になり、WebKit は WebAuthn に transient activation を要求するため `NotAllowedError` になる」。症状（パスキーは iCloud キーチェーンに作成されたのに `NostrKeyInfo` が保存されない＝以後ログインできない）と整合する。**対応済み**: SDK に `NosskeyManager.hasPendingPrf(credentialId, mode)`（同期照会・追加 UV なし）を追加し（`packages/nosskey-sdk/src/nosskey.ts` / `pending-prf-cache.ts`）、`examples/svelte-app` の `AuthScreen.svelte` は create 時に PRF が取れなかった場合に自動で鍵作成へ進まず、**2 タップ目のユーザージェスチャ内で** `createNostrKey()` / `importNostrKey()` を呼ぶ経路（`resumeCreateNew` / `resumeImport`）へ分岐する。PRF が取れる環境（Chrome/Android）の 1 タップ経路は無変更。UA 判定は行わず実測値のみで分岐する。**残: 実機検証** — iOS 実機 + Mac の Web インスペクタで (1) `createPasskey` 直後の `getClientExtensionResults().prf` に `results` が無いこと、(2) 修正前は 2 回目の `credentials.get()` が `NotAllowedError` になること、(3) 修正後は 2 タップで `NostrKeyInfo` が保存されログインできること、を確認したらクローズする。
- [ ] **[iOS] iframe から鍵情報が見えない時間が長い（WebKit のストレージ分離）** — WebKit はクロスオリジン iframe の localStorage を partition し、Storage Access API の grant 後も unpartition されるのは cookie のみ。`examples/svelte-app/src/components/screens/IframeHostScreen.svelte` の `detectInitialState()` はマウント時に**ユーザージェスチャなしで** `document.requestStorageAccess({ all: true })` を呼ぶが、WebKit は原則ジェスチャ必須のため iOS では毎回失敗し、`partitioned` カードのタップを経由しないと鍵が見えない。加えて WebKit の 3rd-party ストレージパーティションは実質エフェメラル（タブを閉じると消える）で、ITP はスクリプト書き込みの localStorage / cookie を 7 日で失効させる。既存の cookie ブリッジ（`services/cookie-storage.ts` / `multi-storage.ts`、Phase 5-A）だけでは埋まらない。鍵が見えない状態はユーザーを汎用ログイン導線へ誘導するため、PR で修正した「ログイン＝復元」の設計と合わせて重要度が高い。**対応方針**: iOS では iframe 内リカバリを諦め、`window.open()` ポップアップ署名方式（P3 の Phase 5-A）へ倒す。あわせて `docs/{ja,en}/iframe-integration` の「Safari / iOS は非対応」記述を実態に合わせて見直す。
