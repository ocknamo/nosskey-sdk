...
後方互換の通常モード維持 (ユーザー承認済: 「普通iframeではないモードはなくてもよい」)
...
Protocol 設計 (packages/nosskey-iframe/src/protocol.ts)
...
Host API (packages/nosskey-iframe/src/host.ts)
...
Client API (packages/nosskey-iframe/src/client.ts)
...
ユーザー回答「普通iframeではないモードはなくてもよい」に従い、Nostr クライアント的機能は削除
追加
...
既存 package.json の SDK 設定 → packages/nosskey-sdk/package.json
修正
...
E2E 手動検証
npm run build モノレポ全体
Svelte アプリ起動 npm run dev -w examples/svelte-app
親用ダミー HTML (ローカル別ポート) 作成し <iframe src="localhost:xxxx/#/iframe"> 埋込
親ページから client.getPublicKey() / client.signEvent(...) 実行
承認ダイアログが iframe 内で出ること、結果が親に返ることを確認
#/settings 単独アクセスでキー作成/エクスポート動作確認
異 Origin 親からの Origin 不一致時に拒否されることを確認
ビルド検証
...
npx biome check .
段階実装順
モノレポ化 (既存 SDK 移動、workspaces 設定、既存テスト通過確認)
nosskey-iframe パッケージ雛形 + protocol.ts + テスト
host.ts 実装 + テスト
client.ts 実装 + テスト
Svelte アプリ改造 (ルーティング, iframe-mode.ts, ConsentDialog)
Svelte アプリ不要機能削除
README 追記, docs/todo.md 更新
全体ビルド/手動検証
各段階でコミット
