...
For a long time, Windows platform authentication (Windows Hello) did not support the PRF extension because it did not support the HMAC-secret extension of CTAP. However, a Windows cumulative update in early 2026 added hmac-secret support, changing the situation. On the Windows side, PRF evaluation is now exposed for both the registration (credential creation) and authentication (assertion) phases through the WebAuthn platform API (`WEBAUTHN_API_VERSION_8`).
