...
| | Windows Hello<br>(Windows 11) | — | Supported (conditional) | hmac-secret support added by a Windows cumulative update in early 2026 (`WEBAUTHN_API_VERSION_8`). Requires a supported Windows 11 build plus a PRF-capable browser (Chrome 147+ / Firefox 148+, etc.) |
...
- Windows Hello gained PRF (hmac-secret) support via a Windows cumulative update in early 2026. Using it requires a supported OS build plus a PRF-capable browser (Chrome 147+ / Firefox 148+, etc.). For older environments, workarounds such as external keys or Google Password Manager remain valid.
