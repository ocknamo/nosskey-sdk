...
長らくWindowsのプラットフォーム認証（Windows Hello）はCTAPのHMAC-secret拡張をサポートしておらずPRF拡張に未対応でしたが、2026年初頭のWindows累積更新でhmac-secret対応が追加され、状況が変わりました。Windows側はWebAuthnプラットフォームAPI（`WEBAUTHN_API_VERSION_8`）を通じて、登録（credential creation）・認証（assertion）の両フェーズでのPRF評価を公開しています。
