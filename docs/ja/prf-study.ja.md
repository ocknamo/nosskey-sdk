# WebAuthn PRF拡張の最新動向と対応状況（2026年5月現在）

## 調査項目

- 現時点（2026年）でPRF拡張が有効なブラウザとそのバージョン
...
WebAuthnのPseudo-Random Function (PRF)拡張は、W3C Web Authentication Level 3仕様で追加された新機能の一つです。このLevel 3仕様は2026年1月13日にW3C勧告候補（Candidate Recommendation）のスナップショットが公開され、最終勧告（Recommendation）に向けた実装フィードバックの段階にあります（勧告への昇格は早くても2026年2月以降の見込み）。
...
現時点（2026年5月）でW3C仕様上はPRF拡張の文言がほぼ確定し、主要ブラウザでの実装も出揃っています。仕様上のステータスは**「Candidate Recommendation（勧告候補）」**であり、W3C WebAuthn WGは各機能の実装報告を集めつつ最終勧告に向けて仕様を完成させようとしています。Level 3ではPRF拡張の他にも複数の拡張（クロスオリジン認証やDevice Public Key拡張等）が追加されています。
...
Mozilla Firefoxについては、2024年～2025年にかけて実装が完了しました。Firefoxではバージョン135（2025年2月4日リリース）でPRF拡張がデフォルト有効（フラグ設定不要）でサポートされ、Windows環境も含め利用可能になっています。Firefox開発チームは「Baseline support」と称して段階的に実装を進めました。その後、Firefox 147で登録（credential creation）時のPRF評価がバックポートされ、Firefox 148以降ではWindows Helloと組み合わせた登録・認証の両フローでPRFがフルサポートされています。したがって、Firefoxをご利用の開発者はバージョン135以上（Windows Helloと併用する場合は148以上）であることを前提とすれば、PRF拡張を用いたWebAuthn機能をテスト・提供できる状況です。

Safari（WebKit）については、Appleが少し遅れて対応を開始しましたが、iOS/iPadOS 18およびmacOS 15 (Sequoia) 以降でPRF拡張に対応しました。具体的には、2024年のWWDCで公開されたSafari 18のベータ版でPRF拡張サポートが実装されており、Safari 18からはWebAuthn PRF拡張が有効になっています。AppleのプラットフォームではエンジンがWebKitに統一されているため、SafariだけでなくiOS上のChromeやFirefoxなど他のブラウザアプリ（内部はWebKitを使用）は、iOS 18以降であればPRF拡張を利用できます。ただしiOS 18リリース初期の18.0～18.3にはクロスデバイス認証時の不具合が報告されており、18.4で修正されています。macOSについても、2024年リリースのmacOS 15 (Sequoia) + Safari 18の組み合わせで正式対応となり、それ以前のSafari 17では未サポートでした。なお、Apple実装のPRFはプラットフォーム認証器（iCloudキーチェーンのパスキー）に限られ、外付けのCTAP2セキュリティキーへはPRF拡張データを受け渡せない制約が残っています。
...
長らくWindowsのプラットフォーム認証（Windows Hello）はCTAPのHMAC-secret拡張をサポートしておらずPRF拡張に未対応でしたが、近年のWindowsでこの状況が変わりました。WindowsのWebAuthnプラットフォームAPIは`WEBAUTHN_API_VERSION_8`でPRF拡張に対応し、登録（credential creation）時のeval値をhmac-secret値へ変換するなど、登録・認証の両フェーズでのPRF評価を公開しています（`WEBAUTHN_AUTHENTICATOR_MAKE_CREDENTIAL_OPTIONS_VERSION_8` 等、Microsoftの`webauthn.h`で確認可能）。ただし、Windows Hello側のPRF対応が具体的にどのWindowsビルド／更新で開始したかはMicrosoft公式には明文化されておらず（コミュニティ報告では2026年初頭の累積更新とされます）、正確な要件は実機での動作確認を推奨します。

ただし、Windows HelloでPRFを利用するには対応ビルドのWindows 11に加えて、ブラウザ側の対応も必要です。ChromeはバージョンによってWindows HelloからのPRFをサーフェスする挙動が異なり、Chrome 147以降で登録時PRFがデフォルト有効化されています（Chrome/Edge 146以前は登録時にPRFをまだサーフェスしません）。FirefoxはFirefox 147で登録時対応がバックポートされ、Firefox 148以降で登録・認証の両方をフルサポートします。

したがって、Windows環境でも対応OSビルドと対応ブラウザを組み合わせればPRF拡張が利用可能になりました。一方、OSやブラウザが要件を満たさない旧環境では引き続きPRFを取得できないため、外付けセキュリティキーの併用などのフォールバックは依然として有効です。

### macOSのTouch ID（およびiCloudキーチェーン上のパスキー）

Appleは最新OSでHMAC-secret機能を組み込んだため、macOS 15 (Sequoia) 以降のTouch ID（Secure Enclave）によるプラットフォーム認証器はPRF拡張をサポートします。macOS 14（Sonoma）以前のTouch IDではPRF非対応でしたが、2024年後半リリースのmacOS 15でサポートが追加されました。これによりSafari 18+ではTouch IDからPRFを取得できます。Chromeなど他ブラウザからも、Appleのプラットフォームパスキー（iCloudキーチェーン）を利用したPRFが可能で、Chrome 132以降でmacOS上のiCloudキーチェーン経由のPRFがサポートされています。
...
以上を踏まえると、2026年時点では主要な認証器（Apple・Googleのプラットフォーム、外部セキュリティキー、そして近年OS側が対応したWindows Hello）がHMAC-secret機能を備えていると言えます。ただしWindows Helloでの利用にはOSの対応ビルドと対応ブラウザ（Chrome 147+／Firefox 148+ 等）の両方が必要で、要件を満たさない組み合わせ（例: Windows Hello + Chrome/Edge 146以前）ではPRFが取得できない点に注意が必要です。開発者は、ユーザーの利用環境によってPRF拡張が利用不可となる組み合わせがある点を踏まえ、フォールバック手段を用意することが望ましいでしょう。その場合は従来通り別途ユーザーに復号用パスワードを入力させる、といった設計上の検討が必要です。Androidやセキュリティキー使用時にはスムーズにPRFが使えるため、各プラットフォームのサポート状況に応じて条件分岐やUI上の案内を行うことが推奨されます。
...
認証情報の作成（登録）時にもextensions.prfを指定可能です。当初のCTAP仕様では登録時にPRF結果を取得できず、認証器がPRF対応かどうか（enabled）のフラグが得られるに留まりましたが、その後「PRF-on-create」への対応が進み、対応環境（例: Chrome 147以降や Firefox 147以降）では登録と同時にPRF結果（results）を取得できるようになっています。いずれにせよ、後続の認証でPRFを得られる認証情報であることを示す指標にもなるため、登録時にPRF拡張をリクエストする価値はあります。以下はパスキー作成時にPRF拡張をリクエストするコード例です。
...
この例ではextensions: { prf: {} }とし、具体的なeval入力は与えていません。この場合getClientExtensionResults().prfにはenabledプロパティ（PRF機能サポートの有無）が返ります（PRF-on-create に対応した環境では results も併せて返されます）。PRFに対応した認証器であれば{ enabled: true, ... }となり、hmac-secret に対応していない認証器では{ enabled: false }となります。この情報を使って、アプリケーション側で「この認証情報は後でデータ復号に使えるか」のフラグを立てておき、もしfalseであればユーザーに別途警告やフォールバック手段を提示するといった処理が可能です。
...
PRF拡張の対応状況は変化が速いため、本文書は定期的な再検証が必要です。
...
- 主要な出典:
  ...
  - [Microsoft webauthn.h（`WEBAUTHN_API_VERSION_8` / hmac-secret 定義）](https://github.com/microsoft/webauthn/blob/master/webauthn.h)
  - 各パスワードマネージャ（Bitwarden / 1Password / Dashlane）の公式ブログ・リリースノート
  - Windows Hello の PRF 対応開始時期は Microsoft 公式に未明文化のため、コミュニティ報告（Corbado / Bitwarden コミュニティ等）を参考にしつつ実機での確認が望ましい
