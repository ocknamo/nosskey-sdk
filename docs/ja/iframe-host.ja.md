...
- `?theme=purple-dark|purple-light|neutral-dark|neutral-light|auto` — `app-state.ts` が適用（旧値 `light`/`dark` も後方互換で受理）
...
- [iframe 埋め込みガイド (親アプリ向け)](./iframe-integration.ja.md) — 親アプリへ iframe を埋め込む手順
