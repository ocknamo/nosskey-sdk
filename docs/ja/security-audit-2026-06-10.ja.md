...
- **対応状況**: ✅ **対応済み（2026-06-24）**。推奨の「decrypt 系は『常に許可』を選べなくする」を採用。`evaluateConsent`（`examples/svelte-app/src/utils/consent-gating.ts`）で `nip44_decrypt` / `nip04_decrypt` は `always` ポリシー・信頼済みオリジンによるサイレント承認の対象外とし（`deny` のみ短絡を許す）、毎回ダイアログを表示する。暗号化（送信内容がダイアログに表示される）とは非対称な不可逆リスクのため復号のみ固定。`ConsentDialog` は復号リクエストで「常に許可」ボタンを出さず注記を表示、`rememberOriginIfRequested`（`iframe-mode.ts`）も多層防御として復号を信頼リストへ記憶しない。設定画面（`ConsentPolicySettings`）に「復号は常に確認される」旨の注記を追加。M-4 のレート制限と併せてオラクル悪用の頻度面も抑制する。
...
- **対応状況**: ✅ **対応済み（2026-06-28、推奨レベルで実装）**。wrap モード（nsec インポート）成功直後に、バックアップファイルの保存を強く促すモーダル（`examples/svelte-app/src/components/WrapKeyBackupPrompt.svelte`）を表示する。インポートは `loginWith` が `setCurrentKeyInfo` → localStorage 永続化を完了してからモーダルを開くため、保存前にタブを閉じても鍵自体は失われない。バックアップ「必須（完全強制）」ではなく「後でやる（設定画面からいつでもエクスポート可）」スキップ導線を残す方針とした（UX 上の離脱防止と、鍵が既に永続化済みである点を踏まえた判断）。PRF 直接モード鍵（パスキーから再導出可能）は対象外（`keyInfo.wrapped` ガード）。
...
- **対応状況**: ✅ **対応済み（2026-06-24）**。`NosskeyIframeHost` にオリジン単位の連続拒否ブロックを実装（`packages/nosskey-iframe/src/host.ts`）。同一オリジンから `maxConsecutiveRejections`（既定 5）回連続で拒否されると、`blockMs`（既定 60 秒）の間それ以降の同意要求をダイアログを出さずに `RATE_LIMITED` エラーで短絡する（iframe も表示しないためフリッカ・連打誘導が起きない）。1 回でも承認されればカウンタはリセット。`requireUserConsent: true` のときのみ作用し、`rateLimit: false` で無効化可能。新エラーコード `RATE_LIMITED` を protocol に追加。SDK パッケージ側の防御なので統合者全員に効く。リクエスト元の固定は H-1（接続承認）＋ `allowedOrigins`（M-5 で必須化）で担保済み。
...
2. **M-1/M-4**: decrypt 系の「常に許可」抑制とレート制限 ✅ 対応済み（2026-06-24）
