...
一方で、**iframe ホストのオリジン許可が実質ノーチェックである点が最大の問題**である。

---

## 🔴 High

### H-1. iframe ホストの `allowedOrigins` が本番コードでも `'*'`（全オリジン許可）

- **箇所**: `examples/svelte-app/src/iframe-mode.ts:119`（本番アプリ）、`packages/nosskey-iframe/src/host.ts:43,76`（SDK デフォルト値も `'*'`）
- **脅威**: ②意図しない操作、①シークレット漏出
- **内容**: nosskey.app として実際にデプロイされるホスト側が、任意のウェブサイトからの postMessage リクエストを受け付ける。コメントで「デバッグ用・本番では限定せよ」と注記されているが、注記された危険なデフォルトのまま本番コードに入っている。影響は二段階。
  1. **`getPublicKey` / `getRelays` は同意ダイアログの対象外**（`host.ts:181-190`、`CONSENT_REQUIRED_METHODS` に含まれない）。つまり**任意の悪意あるサイトが不可視 iframe を埋め込むだけで、ログイン中ユーザーの npub とリレー設定をサイレントに取得**できる。閲覧者の Nostr アカウント特定（デアノニマイズ・トラッキング）がノーインタラクションで成立する。
  2. 署名・暗号化・復号は同意ダイアログで守られるが、任意オリジンがダイアログを出し放題のため、同意疲れや紛らわしい文面による誤承認（さらに「常に許可」チェック → 恒久化）を誘発できる。
- **推奨**: SDK デフォルトを「明示指定必須（未指定なら throw）」に変更。最低でも本番ビルドでは許可リストを環境変数等で注入し、`'*'` をビルド時に拒否する。`getPublicKey`/`getRelays` にも初回オリジン承認（NIP-07 拡張の「接続許可」相当）を導入する。
...
- **脅威**: ①シークレット漏出
- **内容**: `nip44_decrypt`/`nip04_decrypt` の同意ダイアログには相手 pubkey しか表示されず（設計上、復号前に中身を出せないのは妥当）、ユーザーは何を復号するのか判断できない。一度「このサイトを常に許可」を付けると、その origin は**以後無確認で任意の暗号文を復号し放題**になる。信頼済みサイトに XSS が一つあれば、ユーザーの全 DM 履歴を静かに平文化して持ち出せる。H-1（全オリジン許可）と組み合わさると深刻度が跳ね上がる。
...
- **脅威**: ②意図しない操作
- **内容**: origin チェック（H-1 修正後）を通った親は、同意ダイアログを無制限に連打できる。`event.source` への応答は正しく `targetOrigin: event.origin` 指定されており応答漏洩はないが、ダイアログ連打による誤承認誘導（consent fatigue attack）への構造的な歯止めがない。deny カウンタ（`iframe-mode.ts:58-67`）は良い観測手段だが ask 経路には効かない。
...
1. **H-1**: SDK の `allowedOrigins` デフォルト廃止（未指定エラー化）＋本番アプリへの許可リスト導入 — 最優先
