...
```jsonc
{
  ...
  "salt": "6e6f7374722d70776b", // PRF評価入力として使うsalt（hex形式、固定値）
...
saltはPRF評価入力（`prf.eval.first`）としてそのまま使用されます。同じNostr鍵を生成するため、実装間でsalt値を統一する必要があります。標準のsalt値：

```
"nostr-pwk" (UTF-8バイト: 0x6e6f7374722d70776b)
```
