...
- [ ] パスキー作成時にusernameを入力できる機能
- [ ] 一度ログインしたらPWKの値をローカルストレージに保管する。保管されている場合はPRF拡張の対応確認はスキップする。
- [ ] 簡易的なルーティング機能の実装（'/auth', '/nostr'など）
- [ ] モバイルファーストのフッターメニュー追加（画面遷移用）
- [ ] `importNostrKey`機能のサポート追加（現状は`directPrfToNostrKey`のみ）
- [ ] リレー編集が可能な設定画面の追加
- [ ] 設定画面にローカルストレージのクリアボタンを作成
- [ ] 多言語対応の実装（日本語・英語）と設定画面に切替機能を作成
- [ ] 設定画面にシークレットのキャッシュ設定を追加
